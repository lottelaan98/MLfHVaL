...
# isntall tensorflow, keras, numpy, matplotlib if not already installed
...
from keras.layers import Dense, Dropout, Flatten, Conv2D, MaxPooling2D, layers
...
# Rather than splitting the training data to give a validation set, we will pass in the test 
# set as follows: 
# validation_data=(x_test, y_test), 
# Finally, set: 
# shuffle = True  
# to run through the images in a different order of batches each epoch. 
# Execute this model fit command.   

historyc = modelc.fit(
    x_trainc, y_trainc,
    batch_size=32,           # batch size set to 32
    epochs=20,               # run for 20 epochs
    validation_data=(x_testc, y_testc),  # use test set as validation
...
modelm = Sequential()
modelm.add(layers.Input(input_shape=(28, 28, 1))) 
modelm.add(Conv2D(32, kernel_size=(3, 3), activation='relu', padding='same'))
...
historyc = modelm.fit(
    ...
    batch_size=32,           # batch size set to 32
    epochs=20,               # run for 20 epochs
    validation_data=(x_testm, y_testm),  # use test set as validation
    shuffle=True             # shuffle training data each epoch
)
...
# Test loss: 0.019139066338539124
